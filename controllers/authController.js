...
exports.login = catchAsync(async (req, res, next) => {
  ...
  // 1) Check if the email and the password exits
  if (!email || !password)
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!currentUser)
    return next(
      new AppError('The user belonging to this token nolenger exists', 401)
    );
...
exports.restrictTo =
  ...
  (req, res, next) => {
    if (!roles.includes(req.user.role)) {
      return next(
        new AppError('You do not have permissions to perfom this action', 403)
      );
    }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  console.log('req.params', req.params.resetToken);
